...
impl ColWidths {
    const fn for_format(format: RxMessageToStringFormat) -> Self {
        let data = match format {
            RxMessageToStringFormat::Binary => 380.0,
            RxMessageToStringFormat::Hex => 250.0,
            RxMessageToStringFormat::Ascii | RxMessageToStringFormat::Utf8 => 220.0,
        };
        Self {
            timestamp: 110.0,
            cob_id: 90.0,
    ...
    fn info_width(self, table_width: f32, column_spacing: f32) -> f32 {
        const SEPARATOR_COLUMNS: f32 = 5.0;
        const GRID_COLUMNS: f32 = 11.0;
        const SEPARATOR_WIDTH: f32 = 8.0;

        let fixed_width = self.timestamp
            + self.cob_id
            + self.data
            + self.packet_type
            + self.node_id
            + (SEPARATOR_COLUMNS * SEPARATOR_WIDTH)
            + ((GRID_COLUMNS - 1.0) * column_spacing);

        (table_width - fixed_width).max(self.info_min)
    }
}
...
impl MessageRow {
    fn separator(ui: &mut egui::Ui) {
        ...
        let (rect, _) = ui.allocate_exact_size(egui::vec2(8.0, h), egui::Sense::hover());
        ui.painter().text(
            rect.left_center(),
    ...
    pub fn header(&self, ui: &mut egui::Ui, table_width: f32, column_spacing: f32) {
        ...
        Self::separator(ui);
        let info_width = cols.info_width(table_width, column_spacing);
        Self::header_cell(ui, info_width, "Info");
    }
    ...
    pub fn message(
        ...
        self.message_custom_timestamp(ui, d, &self.start_time, table_width, column_spacing);
    }

    pub fn message_custom_timestamp(
        &self,
        ui: &mut egui::Ui,
        d: &MessageCached,
        time: &Instant,
        ...
        Self::separator(ui);
        let info_width = cols.info_width(table_width, column_spacing);
        Self::info_cell(ui, info_width, info, tooltip);
    }
}
